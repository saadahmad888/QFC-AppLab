...
if (counterSwiper) counterSwiper.on('slideChangeTransitionStart', function () {
  ...
  const target = activeCounter.getAttribute("data-target");

  // ✅ FIX: Rebuild digit wrappers based on number length
  const digitsContainer = activeCounter.querySelectorAll(".digit-wrapper");

  // Remove old digit wrappers
  activeCounter.innerHTML = "";

  // Create new digit wrappers based on actual number (NO LEADING ZEROS)
  [...String(target)].forEach(digit => {
