...
const swiperEl = document.getElementById("card-slider");
var slideSwiper, swipertimedelay = 5000;

if (largeMedia.matches) {
  setTimeout(() => {
    swiperEl.classList.remove("pre-init");
    swiperEl.classList.remove("collapsed");
    swiperEl.classList.add("expanded");
  }, 3000);

  setTimeout(() => {
    swiperEl.classList.remove("expanded");
  }, 5000);
  ...
  slideSwiper = new Swiper("#card-slider", {
    ...
    on: {
      init() {
        ...
        // ✅ ensure 2nd slide is active AFTER layout settles
        setTimeout(() => {
          this.slideTo(1, 0); // instant
        }, 50);
      },
      resize() {
        this.update();
...
setTimeout(() => {
  swiperEl.classList.remove("zoom-right");
}, 8200);
